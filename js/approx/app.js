...
var Approximator = (function () {
    ...
    Approximator.prototype.ApproximateAndDrawBezier = function (points) {
        this.points = points;
        ...
        var bez = [points[0], new BABYLON.Vector3(this.x1, this.y1, 0), new BABYLON.Vector3(this.x2, this.y2, 0), points[points.length - 1]];
        return bez;
    };
    Approximator.prototype.CalculateXandY = function () {
    ...
    Approximator.prototype.CalculateParameters = function () {
    ...
    Approximator.prototype.CalculateA1Small = function () {
        this.a1Small = 0;
    ...
    Approximator.prototype.CalculateD1Small = function () {
        this.d1Small = 0;
        for (var i = 0; i < this.points.length; i++) {
    ...
    Approximator.prototype.CalculateA2Small = function () {
        this.a2Small = 0;
        for (var i = 0; i < this.points.length; i++) {
    ...
    Approximator.prototype.CalculateD2Small = function () {
        this.d2Small = 0;
        for (var i = 0; i < this.points.length; i++) {
    ...
    Approximator.prototype.CalculateBSmall = function () {
        this.bSmall = 0;
        for (var i = 0; i < this.points.length; i++) {
            this.bSmall = this.bSmall + this.b_arr[1][i] * this.b_arr[1][i];
        }
    };
    Approximator.prototype.CalculateCSmall = function () {
        this.cSmall = 0;
        for (var i = 0; i < this.points.length; i++) {
            this.cSmall = this.cSmall + this.b_arr[2][i] * this.b_arr[1][i];
        }
    };
    Approximator.prototype.CalculateESmall = function () {
        this.eSmall = 0;
        for (var i = 0; i < this.points.length; i++) {
            this.eSmall = this.eSmall + this.b_arr[2][i] * this.b_arr[2][i];
        }
    };
    Approximator.prototype.CalculateLength = function () {
        this.lengthSumm = 0;
        this.length = [];
        for (var i = 0; i < this.points.length - 1; i++) {
            this.length[i] = this.lengthSumm;
            var currLength = Math.sqrt(Math.pow(this.points[i + 1].x - this.points[i].x, 2) + Math.pow(this.points[i + 1].y - this.points[i].y, 2));
            this.lengthSumm += currLength;
        }
        this.length[this.points.length - 1] = this.lengthSumm;
    };
    Approximator.prototype.CalculateBArr = function () {
        this.b_arr = [];
        for (var i = 0; i < 4; i++) {
            this.b_arr[i] = [];
            for (var j = 0; j < this.points.length; j++) {
                console.log(i, this.length[j], this.lengthSumm);
                var num = this.B(i, this.length[j] / this.lengthSumm);
                console.log(num);
                this.b_arr[i][j] = this.B(i, this.length[j] / this.lengthSumm);
            }
        }
    };
    Approximator.prototype.B = function (j, i) {
        switch (j) {
            case 0:
                return this.B0(i);
            case 1:
    ...
    Approximator.prototype.B0 = function (arg) {
        return Math.pow(1 - arg, 3);
    };
    Approximator.prototype.B1 = function (arg) {
        return 3 * Math.pow(1 - arg, 2) * arg;
    };
    Approximator.prototype.B2 = function (arg) {
        return 3 * (1 - arg) * Math.pow(arg, 2);
    };
    Approximator.prototype.B3 = function (arg) {
        return Math.pow(arg, 3);
    };
    return Approximator;
}());
var ApproximateCurveGraph = (function (_super) {
    __extends(ApproximateCurveGraph, _super);
    function ApproximateCurveGraph(name, points, scene, graphIndex) {
        if (graphIndex === void 0) { graphIndex = 0; }
        _super.call(this, name, points, scene, graphIndex);
        this.approximator = new Approximator();
    }
    ApproximateCurveGraph.prototype.bernstein = function (n, i, t) {
        var binom = 1;
        for (var x = n - i + 1; x <= n; x++)
            binom *= x;
        for (var x = 1; x <= i; x++)
            binom /= x;
    ...
    ApproximateCurveGraph.prototype.vectorAtT = function (vertices, t) {
        var n = vertices.length;
        var res = new BABYLON.Vector3(0, 0, 0);
        for (var i = 0; i <= n - 1; i += 1) {
            var b = this.bernstein(n - 1, i, t);
            //console.log("b", b, "vertex", vertices[i]);
            var next = vertices[i].multiplyByFloats(b, b, b);
            res.addInPlace(next);
        }
        return res;
    };
    ApproximateCurveGraph.prototype.arbitraryBezier = function (vertices, nb) {
        var bez = [];
        var step = 1 / nb;
        for (var i = 0; i <= 1; i += step) {
    ...
    ApproximateCurveGraph.prototype.initCurves = function () {
        ...
        var actualPoints = this.approximator.ApproximateAndDrawBezier(this.points);
        var curvePoints = this.arbitraryBezier(actualPoints, 50);
        this.curves[0] = BABYLON.Mesh.CreateLines("name", curvePoints, this.scene, true);
    };
    ApproximateCurveGraph.prototype.updateSpline = function (pointIndex) {
        if (this.points.length < 4) {
            return;
        }
        var actualPoints = this.approximator.ApproximateAndDrawBezier(this.points);
    ...
    return ApproximateCurveGraph;
}(BaseCurveGraph));
var LongApproximateCurvePlance = (function (_super) {
    __extends(LongApproximateCurvePlance, _super);
    function LongApproximateCurvePlance(name, width, height, scene, graph) {
        _super.call(this, name, 1300, 600, scene, graph);
        this.graph = new ApproximateCurveGraph("approximate", [], scene, 0);
    }
    return LongApproximateCurvePlance;
}(SimpleCurvePlane));
var ApproximateCurveEditor = (function () {
    function ApproximateCurveEditor(name, scene) {
        this.scene = scene;
        this.name = name;
        this.curvePlane = new LongApproximateCurvePlance("plane", 0, 0, scene);
    }
    ApproximateCurveEditor.prototype.onPointerDown = function (evt, pickingInfo) {
        this.curvePlane.onPointerDown(evt, pickingInfo);
    };
    ApproximateCurveEditor.prototype.onPointerUp = function (evt, pickingInfo) {
        this.curvePlane.onPointerUp(evt, pickingInfo);
    };
    ApproximateCurveEditor.prototype.onPointerMove = function (evt, pickingInfo) {
        this.curvePlane.onPointerMove(evt, pickingInfo);
    };
    ApproximateCurveEditor.prototype.saveJsonData = function () {
        return this.curvePlane.saveJsonData();
    };
    ApproximateCurveEditor.prototype.loadJsonData = function (json) {
        this.curvePlane.loadJsonData(json);
    };
    ApproximateCurveEditor.prototype.loadVectorData = function (vectorData) {
        this.curvePlane.loadVectorData(vectorData);
    };
    return ApproximateCurveEditor;
}());
...
var createScene = function () {
    ...
    editor = new ApproximateCurveEditor("editor", scene);
